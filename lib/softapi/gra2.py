...
g2 = tf.Graph()
sess2 = tf.Session(graph=g2,config=tf.ConfigProto(intra_op_parallelism_threads=1,inter_op_parallelism_threads=1))
with g2.as_default():
        class layer:
                empCount = 0
...
def gra_n2(co,n1l):
        ...
        Dif = 2/Dif 
        gra = np.array(sess2.run(gradient,feed_dict=d)).reshape(Atnum,Gnum)
        gra = gra*(0.5*Edif)### Energy was NORMALIZED!!
        gra = np.multiply(gra,Dif)
        pre = np.array(sess2.run(prediction,feed_dict=d),dtype=float)
